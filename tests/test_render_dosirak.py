...
def test_own_news_has_two_groups_and_cards_use_300px_image(letter):
    ...
    assert 'width="282" data-lf="card"' in letter and "max-width:282px" in letter
    assert 'width="300"' not in letter and "max-width:312px" not in letter and "max-width:300px" not in letter
    # Stibee HTML block is 600px: card row has no side padding, 3px border + two 294px columns (mso table too), no whitespace between them
    assert letter.count('<tr><td style="padding:0;"><table role="presentation" width="100%"') == 2
    assert letter.count('display:inline-block;width:100%;max-width:294px;vertical-align:top;') == 4
    assert '<td width="294" valign="top">' in letter and letter.count('<td width="294"') == 4
    assert re.search(r'</div>\s+<div style="display:inline-block', letter) is None
    assert 'style="display:block;width:100%;max-width:282px;height:auto;border:0;margin:6px auto;"' in letter
